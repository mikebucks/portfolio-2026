...
// two featured cards side by side, or a handful of rows on a fast scroll —
// cascade with the modal's 0.18s stagger, in document order, so the page and
// the case study share one motion vocabulary.
//
// Two moves, chosen per element by the attribute's value:
//   data-reveal          rise: 24px down → in place (the modal's own move)
//   data-reveal="scale"  grow: 0.9 → 1 about the centre (featured cards)
// Both fade in alongside.
//
...
const kindOf = (el: HTMLElement) =>
  el.dataset.reveal === "scale" ? "scale" : "rise";
      ...
      (entries) => {
        ...
        batch.forEach((el, i) => {
          ...
          gsap.to(el, {
            ...
            // open a stacking context on the featured cards, which need to
            // compete at page level for their hover ring (see FeaturedProjects).
